...
function ComposeSection({onSuccess}:{onSuccess:()=>void}){
  const [url,setUrl]=useState("");const [cat,setCat]=useState("AUTO");const [preview,setPreview]=useState<Preview|null>(null);const [loading,setLoading]=useState(false);const [posting,setPosting]=useState(false);const [err,setErr]=useState<string|null>(null);const [ok,setOk]=useState<string|null>(null);const [lightbox,setLightbox]=useState(false);const [copied,setCopied]=useState<string|null>(null);const [editTitle,setEditTitle]=useState("");const [editCaption,setEditCaption]=useState("");const [editing,setEditing]=useState(false);
  useEffect(()=>{setErr(null);setPreview(null);setOk(null);setEditing(false);},[url]);
  useEffect(()=>{if(preview){setEditTitle(preview.ai.clickbaitTitle);setEditCaption(preview.ai.caption);}},[preview]);
  async function doPreview(){if(!url.trim())return;setLoading(true);setErr(null);setPreview(null);setOk(null);try{const body:Record<string,string>={url:url.trim()};if(cat!=="AUTO")body.category=cat;const r=await fetch("/api/preview-url",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||"Preview failed");setPreview(d);}catch(e:any){setErr(e.message);}finally{setLoading(false);}}
  async function doPost(){if(!preview)return;setPosting(true);setErr(null);setOk(null);try{const body:Record<string,string>={url:url.trim()};if(cat!=="AUTO")body.category=cat;if(editing&&editTitle)body.manualTitle=editTitle;if(editing&&editCaption)body.manualCaption=editCaption;const r=await fetch("/api/post-from-url-proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const d=await r.json();if(!r.ok||d.error)throw new Error(d.error||"Post failed");const ig=d.instagram?.success,fb=d.facebook?.success;setOk((ig&&fb)?"Posted to IG + FB":ig?"Posted to IG only":fb?"Posted to FB only":"Both failed");if(ig||fb){setUrl("");setPreview(null);setTimeout(onSuccess,1500);}}catch(e:any){setErr(e.message);}finally{setPosting(false);}}
  ...
  return <div style={{maxWidth:560}}><div style={{marginBottom:16}}><h2 style={{fontFamily:"Bebas Neue,sans-serif",fontSize:22,letterSpacing:1,marginBottom:4}}>Compose</h2><p style={{fontSize:12,color:"#555"}}>Paste any URL, preview with AI, edit if needed, then post.</p></div>
    <div style={{display:"flex",gap:8,marginBottom:12}}><input style={{...inp,flex:1}} placeholder="Paste article / YouTube / TikTok URL..." value={url} onChange={e=>setUrl(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doPreview()}/><button onClick={doPreview} disabled={!url.trim()||loading} style={{background:"#E50914",color:"#fff",border:"none",borderRadius:6,padding:"10px 18px",fontSize:13,fontWeight:700,cursor:"pointer",opacity:(!url.trim()||loading)?.4:1,whiteSpace:"nowrap"}}>{loading?<Spin/>:"Preview"}</button></div>
    ...
    {preview&&<div style={{background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:10,overflow:"hidden"}}>
      ...
      <div style={{padding:16}}>
        {editing?<><input value={editTitle} onChange={e=>setEditTitle(e.target.value)} style={{...inp,marginBottom:8,fontFamily:"Bebas Neue,sans-serif",fontSize:18,letterSpacing:.5}}/><textarea value={editCaption} onChange={e=>setEditCaption(e.target.value)} rows={5} style={{...inp,resize:"vertical",marginBottom:12,lineHeight:1.6}}/><button onClick={()=>setEditing(false)} style={{background:"#2a2a2a",color:"#aaa",border:"none",borderRadius:6,padding:"7px 14px",fontSize:12,cursor:"pointer",marginBottom:12}}>Done editing</button></>
